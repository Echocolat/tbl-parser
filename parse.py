...
1:['Boolean',unpack_value, [1, '<?']],
2:['Byte', unpack_value, [1, '<b']],
3:['Char', unpack_value, [1, '<c']],
5:['Decimal', LengthPrefixedString, 0],
6:['Double', unpack_value, [8, '<f']],
...
def ClassTypeInfo(s):
    classTypeInfo = {}
    TypeName = LengthPrefixedString(s).decode('utf-8')
...
def BinaryLibrary(s):
    binaryLibrary = {}
    LibraryId = struct.unpack('<I', popp(s, 4))[0]
    LibraryName = LengthPrefixedString(s).decode('utf-8')
...
def ClassInfo(s):
    ...
    Name = LengthPrefixedString(s).decode('utf-8')
    MemberCount = struct.unpack('<I', popp(s, 4))[0]
    MemberNames = []
    for i in range(MemberCount):
        MemberNames.append(LengthPrefixedString(s).decode('utf-8'))
...
def BinaryObjectString(s):
    binaryObjectString = {}
    ObjectId = struct.unpack('<I', popp(s, 4))[0]
    Value = LengthPrefixedString(s).decode('utf-8')
    printverbose('\tObjectId : 0x%x'%ObjectId)
...
def MethodCall(s):
    ...
    MethodName = StringValueWithCode(s).decode('utf-8')
    TypeName = StringValueWithCode(s).decode('utf-8')
...
def parse(fileName):
    ...
    with open(fileName.replace('.tbl', '.json').replace('tbl/', 'json_raw/'), 'w') as json_file:
        json_file.write(json.dumps(myObject, indent = 2))
...
def main():
    ...
    if len(sys.argv) != 1:
        try:
            ...
            input('An error occured. Make sure your .tbl file is not corrupted. Press enter to quit. ')

    else:
        input('Call the script with the .tbl file as an argument. Press enter to exit. ')
